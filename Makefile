...
run:
	docker run -it -e DISPLAY=$(DISPLAY) -v /tmp/.X11-unix:/tmp/.X11-unix -v $(HOME)/.Xauthority:/home/$(USER)/.Xauthority --net=host --device=/dev/datadev_0:/dev/datadev_0 assert_dev_image:$(shell git branch --show-current)
